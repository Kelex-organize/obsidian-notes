...
U = [2 1 -1;
     ...
     0 0 4];
b = [3; 9; 8];
x = sustitucion_hacia_atras(U,b)'      # x = [1.6667, 1.6667, 2.0000]
```
...
# Estabilidad y Convergencia
 Para los métodos de directos de resolución de sistemas lineales, la única fuente de error es la representación de número reales con precisión finita.
### Norma de vectores y matrices
Definicion: Sea $|| \cdot ||_v$ una norma vectorial. 
- **Norma inducida:** la norma matricial inducida por ||\cdot||_v
- **Normas compatibles:** decimos que $||\cdot||_v$ es compatible con $||\cdots||_v$ si cumple que:
$$||A\mathbf x ||_v \le ||A||_M ||\mathbf x ||_v \qquad \forall A\in M_{m\times n}(\mathbb R)^n$$
- **Normas submultiplicativas:** decimos que $||\cdot ||_M$ es multiplicativa si se cumple que: 
$$||AB||_M \le ||A||_M ||B||_M, \qquad \forall A\in M_{m\times n}(\mathbb R), B\in M_{m\times n}(\mathbb R)$$
