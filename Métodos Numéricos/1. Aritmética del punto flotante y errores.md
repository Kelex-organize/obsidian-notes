...
$$|\varepsilon_x| = \frac{fl(x) - x}{|x|} \le \frac{\varepsilon_M}{2} \quad \Longrightarrow \quad |e_x| = |x \varepsilon_x| \le \frac{|x|\varepsilon_M}{2}$$
...
```octave
(1 + 1e-16) + 1e-16 - 1     # ans = 0
1 + (1e-16 + 1e-16) - 1     # ans = 2.2204e-16
(1 + 1e-16) + (1e-16 - 1)   # ans = 1.1102e-16
```
...
Sea $f: \mathbb R \rightarrow \mathbb R$, de clase $C^2$. La definición de la derivada de $f$ en $x \in R$,
$$f'(x) =\lim_{h \rightarrow0} \Delta_h f(x), \quad \Delta_h f(x) := \frac{f(x+h) -f(x)}{h}$$
hola
$$|e_{f'(x)}| = |f'(x) - \frac{\text{fl}(f(x+h) - \text{fl}f(x))}{h} \le
|f'(x)−\Delta_hf(x)| + \Delta_h f(x) − \frac{fl(f(x + h)) − fl(f(x))}{h}
$$
$$=: |\text{error}_{\text{truncamiento}}| + |\text{error}_{\text{redonde}}|$$




- **Error de truncamiento:** $|\text{error}_{\text{truncamiento}}| \approx |f''(x)| \frac{h}{2}$
- **Error de punto flotante:**$|\text{error}_{\text{redondeo}}| \lesssim |f(x)| \frac{\varepsilon_M}{h}$
Obs: El error de truncamiento aumenta segun
...
$$I_n = \int_0^1 \frac{x^n}{5 + x} dx$$
Rescribamos la integral de manera que sea mas sencillo resolverla:
$$\frac{x^n}{5 + x} = \frac{x^{n - 1} \cdot x}{5 + x} = \frac{x^{n - 1} \cdot (x + 5 - 5)}{5 + x} = \frac{x^{n - 1} \cdot (x + 5) - 5x^{n-1}}{5 + x} = x^{n-1} - \frac{5x^{n-1}}{5 + x}$$
Por los que la integral nos quedo de la manera:
$$I_n = \int_0^1 x^{n-1} - \frac{x^{n-1}}{5 + x} dx = \int_0^1x^{n-1} - 5\int_0^1\frac{x^{n-1}}{5 + x}$$
Observemos que 
$$I_{n-1} = \int_0^1\frac{x^{n-1}}{5 + x}$$
Por lo que tenemos que la integral se puede escribir de la siguiente manera:
$$I_n = \int_0^1 x^{n-1} - 5I_{n-1} \quad \Longrightarrow \quad I_n - 5I_{n-1} = \int_0^1 x^{n-1}$$
Resolviendo la integral que queda 
$$\int_0^1 x^{n-1} = \frac{1}{n}\left[ x^n\right]^1_0 = \frac{1}{n} (1-0) = \frac{1}{n}$$
Llegando a la expresión
$$I_n - 5 I_{n-1} = \frac{1}{n}$$
Analicemos el signo de $I_n$. 
$$I_n = \frac{1}{n} + 5 I_{n-1}$$
Como $I_n$ se puede escribir como la suma de dos términos analicemos el signo de estos
- $1/n$ es positivo para todo n.
- $5I_{n-1}$ lo podemos escribir como 
...
---
### Ejercicio 12  
La Extrapolación de Richardson permite mejorar el orden del error de truncamiento de una estimación numérica. Supongamos que $p^∗$ es un número a estimar y tenemos una estimación $p_0(h)$ que cumple 
$$\tag 1 p^∗ = p_0(h) + a_0h^{k_0} + a_1h^{k_1} + \cdots$$ 
donde las $a_i \in \mathbb R$ son constantes desconocidas y las potencias $k_i > 0$ son conocidas y cumplen $k_0 < k_1 < k_2 < \cdots$. La identidad de arriba puede ser interpretada como que el error de truncamiento al aproximar $p^∗$ mediante $p_0(h)$ es del orden de $h^{k_0}$ . Buscamos ahora reutilizar $(1)$ para producir una nueva aproximación $p_1(h)$ de $p^∗$ cuyo error de truncamiento sea del orden de $h^{k_1}$ . Con este fin, dado $t > 0$ consideramos el estimador $p_0$ evaluado en $h/t$ y utilizamos $(1)$: 
$$p^∗ = p_0(h/t) + a_0 \frac{h^{k_0}}{t^{k_0}} + a_1 \frac{h^{k_1}}{t^{k_1}} + \cdots$$ La extrapolación de    Richardson consiste en combinar esta identidad con $(1)$, para obtener un nuevo estimador $p_1(h)$ de $p^∗$ , 
$$p_1(h) = αp_0(h) + βp_0(h/t)$$de modo que el error de truncamiento al aproximar $p^∗$ mediante $p_1(h)$ sea del orden de $h^{k_1}$ , esto es, 
$$p^∗ = p_1(h) + a_1h^{k_1} + . . .$$
1. Hallar $α$ y $β$ y verificar que es 
$$p_1(h) = \frac{t^{k_0} p_0(h/t) − p_0(h)}{ t^{k_0} − 1}$$
**Solución:**

2. Se sabe que $\lim_{h→0}(1 + h)^{1/h} = e$, y que ésta es una fórmula de primer orden 
$$e = (1 + h) 1/h + a0h + . . .$$
...
3. Verificar los resultados de la parte anterior en Octave: para $h = 10^{−2} , 10^{−3} , 10^{−4}$ , comparar los errores cometidos por la aproximación $e ≃ (1 + h)^{1/h}$ con los cometidos al usar el método de la parte anterior.
**Solución:**
